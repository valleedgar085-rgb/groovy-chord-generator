...
export function generateChordFromFunction(
  ...
  // Two separate mappings are needed for chord calculation:
  // 1. degreeToSemitones: Maps degree symbols to chromatic intervals (semitones) from the root
  //    Used to transpose the root note to the correct pitch for the chord
  //    Example: 'V' = 7 semitones (a perfect fifth above the root)
  ...
  // 2. degreeToIndex: Maps degree symbols to scale position indices (0-6)
  //    Used to look up the roman numeral display name in ROMAN_NUMERALS array
  //    Example: 'V' = 4 (the 5th scale degree, 0-indexed)
